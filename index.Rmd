...
SMTM_plot_tempo_popularity <- ggplot(SMTM, aes(y=tempo, x=valence, label=track.name))+ geom_point(aes(color=playlist_name)) 
ggplotly(SMTM_plot_tempo_popularity)
```
...
```{r}
HSR_plot_tempo_popularity <- ggplot(HSR, aes(y=tempo, x=valence, label=track.name))+ geom_point(aes(color=playlist_name)) 
ggplotly(HSR_plot_tempo_popularity)
```
